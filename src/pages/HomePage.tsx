...
const HomePage: React.FC = () => {
	...
	const { data: projects, loading: loadingP } = useApiData<Project[]>(
		"/products",
		dummyProjects,
	);
	...
	// カルーセルの初期化と無限スクロール
	useEffect(() => {
		if (!carouselRef.current || !Array.isArray(projects) || !projects.length)
			return;
		...
		const startAutoScroll = () => {
			...
			intervalRef.current = window.setInterval(() => {
				if (!carouselElement || isScrollingRef.current || isDraggingRef.current)
					return;

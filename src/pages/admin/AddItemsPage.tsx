...
const AddItemsPage = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const payload = {
            ...
            category_ids: categoryInput.split(",").map((value) => value.trim()).filter(Boolean),
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <label>
                    <span>image_url</span>
                    <textarea value={imageInput} onChange={(event) => setImageInput(event.target.value)} />
                </label>
                <button type="submit" disabled={submitting}>{submitting ? "送信中..." : "追加する"}</button>
            </form>
        </div>
    );
};
